// src/constants/koreaRegions.ts

export const normalizeSido = (s: string) => {
  // 표 기준으로 강제 통일
...
export const normalizeSigungu = (sido: string, sigungu: string) => {
  const sidoN = normalizeSido(sido);

  // 세종은 표에서 sigungu도 "세종특별자치시"로 내려옴
  if (sidoN === '세종특별자치시') {
    if (sigungu === '세종시' || sigungu === '세종') return '세종특별자치시';
  }
  return sigungu;
};
...
export const DISTRICT_OPTIONS: Record<string, string[]> = {
  ...
  경기도: [
    // 표에 있는 것만 두는 게 제일 안전(서버 검증이 표 기반이면)
    '가평군','고양시','과천시','광명시','광주시','구리시','군포시','김포시','남양주시','동두천시',
    '부천시','성남시','시흥시','안산시','안성시','안양시','양주시','양평군','여주시','연천군',
